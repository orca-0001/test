/**
 * @file   common_algorithm.h
 * @brief  提供嵌入式开发中经常使用的算法，采用标准C编写。
 * @version V2.0.0
 * @date 2023-04-14
 * @author twen51
 * @copyright Copyright (c) 2023 twen51
 */

#ifndef _COMMON_ALGORITHM_H
#define _COMMON_ALGORITHM_H
 ...
 * @return 累加和
 */
unsigned char ca_get_sum_check(const void* ptr,unsigned long size) {
...
const unsigned char ca_crc8tab[256] = {
 ...
 * @return 错误代码
 */
unsigned char ca_get_crc8(const unsigned char* ptr,unsigned long size) {
  unsigned char crc = 0;
  unsigned long i;
  for (i = 0; i<size; i++) {
    crc = ca_crc8tab[crc^*ptr++];
  }
  return crc;
}

// CRC8 implementation with polynom = 0xBA
const unsigned char ca_crc8tab_BA[256] = {
 ...
 * @return 错误代码
 */
unsigned char ca_get_crc8_BA(const unsigned char* ptr,unsigned long size) {
  unsigned char crc = 0;
  unsigned long i;
  for (i = 0; i<size; i++) {
    crc = ca_crc8tab_BA[crc^*ptr++];
  }
  return crc;
}

const unsigned char ca_aucCRCHi[] = {
...
const unsigned char ca_aucCRCLo[] = {
 ...
 * @return 错误代码
 */
unsigned int ca_get_crc16(const void* ptr,unsigned long size) {
  const unsigned char* data = (const unsigned char*)ptr;
  ...
  while (size--) {
    iIndex = ucCRCLo^*(data++);
    ucCRCLo = (unsigned char)(ucCRCHi^ca_aucCRCHi[iIndex]);
    ucCRCHi = ca_aucCRCLo[iIndex];
  }
  return (unsigned int)(ucCRCHi<<8|ucCRCLo);
}

const unsigned long ca_crc32tab[256] = {
 ...
 * @return 错误代码
 */
unsigned long ca_get_crc32(const void* ptr,unsigned long size) {
  const unsigned char* data = (const unsigned char*)ptr;
  unsigned long crc = size;
  unsigned long i;
  for (i = 0; i<size; i++)
    crc = (crc>>8)^ca_crc32tab[(crc&0xff)^data[i]];
  return crc;
}
 ...
 * @return 错误代码
 */
unsigned long ca_get_djb2(const void* ptr,unsigned long size) {
  const unsigned char* data = (const unsigned char*)ptr;
